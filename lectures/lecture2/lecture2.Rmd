...
We'll use the `cement` dataset. The story is that you want to see whether two
different chemical formulas lead to cement that have the same strength. First,
let's read in the data and make a plot.
...
It looks like the unmodified cement is stronger. But it's easy to be tricked
into seeing patterns which are explainable by pure randomness, so a test would
still be worthwhile. We'll assume that the variance $\sigma^2$ is the same in
both groups, but will still need to estimate. So, we will use a $t$-test. A
priori, we didn't have a guess about whether one formula was stronger than the
other, so we'll test against the two-sided alternative $H_{a1}$.
...
&= \frac{1}{n_1}\sigma_1^2 + \sigma{1}{n_2}\sigma_2^2.
$$
...
$$
\nu\left(\left[ \frac{\frac{s_1^2}{n_1} + \frac{s_2^2}{n_2}}{\frac{s_1^2}{n_1} + \frac{s_2}     \right])
$$

$$
\left[(\frac{\left(\frac{s_1^2}{n1}\right) ^ 2}{n_1 - 1} + \frac{\left(\frac{s_2^2}{n_2}\right) ^ 2}{n_1 - 1}\right]
$$

 [(s1^2 / n1) ^ 2 / (n1 - 1) + (s2^2 / n2) ^ 2 / (n2 - 1)]

In this case, we should not used the pooled standard error estimator $s_{p}^{2}$
defined above. 

In this case, the variance of the difference is changed, but can still obtain a
$N(0, 1) reference distribution with appropriate standardization,
